...
const Icons = {
  ...
  rust: <img src="/rust.png" alt="Rust" className="h-16 w-auto object-contain translate-y-1" />,
      ...
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              <input
                ...
                className="w-full px-5 py-4 bg-neutral-900/80 rounded-2xl border border-neutral-800 focus:border-orange-500/50 focus:outline-none transition-all text-lg placeholder:text-neutral-500"
              />
            </div>
            <button
              onClick={() => handleSearch()}
              disabled={loading}
              className="px-8 py-4 bg-gradient-to-r from-red-600 to-orange-500 hover:from-red-500 hover:to-orange-400 text-white rounded-2xl font-semibold disabled:opacity-50 transition-all flex items-center gap-2 shadow-lg shadow-orange-500/20"
            >
              {loading ? (
        ...
        {data && (
          <div className="space-y-6 mb-10">
            {/* Player Card */}
            <div className="p-6 bg-gradient-to-br from-neutral-900 to-neutral-950 backdrop-blur rounded-3xl border border-orange-500/20 shadow-xl shadow-orange-500/5">
              ...
              {data.stats && (
                <div className="mt-6 pt-6 border-t border-orange-500/10">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="p-4 bg-gradient-to-br from-orange-500/10 to-red-500/5 rounded-2xl border border-orange-500/10">
                      <div className="flex items-center gap-2 text-orange-300/70 text-sm mb-1">
                        {Icons.clock}
                        <span>Часов в Rust</span>
                      </div>
                      <p className="text-2xl font-bold text-orange-400">{data.stats.playtimeHours}h</p>
                    </div>
                    <div className="p-4 bg-neutral-800/50 rounded-2xl border border-neutral-700/30">
                      <div className="flex items-center gap-2 text-gray-400 text-sm mb-1">
                        {Icons.shield}
                        <span>VAC баны</span>
                      </div>
                      <p className={`text-2xl font-bold ${data.stats.vacBans > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
                        {data.stats.vacBans}
                      </p>
                    </div>
                    <div className="p-4 bg-neutral-800/50 rounded-2xl border border-neutral-700/30">
                      ...
                      <p className={`text-2xl font-bold ${data.stats.gameBans > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
                        {data.stats.gameBans}
                      </p>
                    </div>
                    {data.stats.daysSinceLastBan !== undefined && (data.stats.vacBans > 0 || data.stats.gameBans > 0) && (
                      <div className="p-4 bg-neutral-800/50 rounded-2xl border border-neutral-700/30">
                        ...
                        <p className="text-2xl font-bold text-amber-400">{data.stats.daysSinceLastBan}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Auto Refresh */}
              <div className="mt-6 pt-6 border-t border-orange-500/10 flex items-center gap-4 flex-wrap">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <div className={`relative w-12 h-6 rounded-full transition-colors ${autoRefresh ? 'bg-gradient-to-r from-red-500 to-orange-500' : 'bg-neutral-700'}`}>
                    <input type="checkbox" checked={autoRefresh} onChange={(e) => setAutoRefresh(e.target.checked)} className="sr-only" />
                    <div className={`absolute top-1 w-4 h-4 rounded-full transition-all ${autoRefresh ? 'left-7 bg-white' : 'left-1 bg-neutral-400'}`} />
                  </div>
                ...
                {autoRefresh && (
                  <>
                    <select
                      ...
                      className="bg-neutral-800 rounded-xl px-4 py-2 text-sm border border-orange-500/20 focus:border-orange-500/50 focus:outline-none"
                    >
            ...
            {data.player.gameid === '252490' ? (
              data.server ? (
                <div className="p-6 bg-gradient-to-br from-orange-950/50 to-red-950/30 border border-orange-500/30 rounded-3xl">
                  <div className="flex items-center gap-3 mb-5">
                    <span className="w-3 h-3 bg-orange-500 rounded-full animate-pulse shadow-lg shadow-orange-500/50" />
                    <span className="text-orange-400 font-semibold text-lg">Сейчас играет в Rust</span>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center gap-3 p-3 bg-black/30 rounded-xl border border-orange-500/10">
                      {Icons.server}
                      <div>
                        <p className="text-xs text-orange-300/50">Сервер</p>
                    ...
                    <div className="flex items-center gap-3 p-3 bg-black/30 rounded-xl border border-orange-500/10">
                      {Icons.users}
                      <div>
                        <p className="text-xs text-orange-300/50">Игроки</p>
                    ...
                    <div className="flex items-center gap-3 p-3 bg-black/30 rounded-xl border border-orange-500/10">
                      {Icons.map}
                      <div>
                        <p className="text-xs text-orange-300/50">Карта</p>
                    ...
                    <div className="flex items-center gap-3 p-3 bg-black/30 rounded-xl border border-orange-500/10">
                      {Icons.globe}
                      <div>
                        <p className="text-xs text-orange-300/50">IP адрес</p>
                        <p className="font-medium font-mono">{data.server.ip}:{data.server.port}</p>
                      </div>
                    </div>
                  </div>
                  {data.server.url && (
                    <a href={data.server.url} target="_blank" className="inline-flex items-center gap-2 mt-5 px-5 py-2.5 bg-gradient-to-r from-red-600/20 to-orange-500/20 hover:from-red-600/30 hover:to-orange-500/30 border border-orange-500/30 rounded-xl text-orange-400 transition-colors">
                      Открыть на BattleMetrics {Icons.external}
                    </a>
                  )}
                </div>
              ) : (
                <div className="p-6 bg-gradient-to-br from-amber-950/30 to-yellow-950/20 border border-amber-500/30 rounded-3xl">
                  <p className="text-amber-400 font-medium">Играет в Rust, но сервер скрыт или не найден</p>
                  {data.player.gameserverip && <p className="text-gray-400 mt-2 font-mono">IP: {data.player.gameserverip}</p>}
                </div>
              )
            ) : data.player.gameextrainfo ? (
              <div className="p-6 bg-gradient-to-br from-neutral-800/50 to-neutral-900/50 border border-neutral-600/30 rounded-3xl">
                <p className="text-neutral-300 font-medium">Сейчас играет в: {data.player.gameextrainfo}</p>
              </div>
            ) : (
              <div className="p-6 bg-neutral-900/50 border border-neutral-800/50 rounded-3xl">
                <p className="text-gray-500">Не в игре</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {favorites.map(player => (
                <div
                  key={player.steamid}
                  className="group flex items-center gap-4 p-4 bg-gradient-to-br from-neutral-900 to-neutral-950 rounded-2xl border border-orange-500/20 hover:border-orange-500/50 cursor-pointer transition-all hover:shadow-lg hover:shadow-orange-500/10"
        ...
        {history.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-semibold flex items-center gap-2 text-neutral-400">
                {Icons.clock} История поиска
              </h3>
              <button onClick={clearHistory} className="flex items-center gap-1 text-sm text-neutral-500 hover:text-red-400 transition-colors">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {history.map(player => (
                <div
                  key={player.steamid}
                  className="flex items-center gap-3 p-3 bg-neutral-900/50 rounded-xl border border-neutral-800/50 hover:border-orange-500/30 hover:bg-neutral-900 cursor-pointer transition-all"
                  onClick={() => { setSteamUrl(player.url); handleSearch(player.url); }}
                >
                  <img src={player.avatar} alt="" className="w-10 h-10 rounded-lg" />
                  <span className="flex-1 truncate text-sm text-neutral-300">{player.name}</span>
                </div>
              ))}
            </div>
          </div>
